...
nbModel = MultinomialNB(alpha=0.1, fit_prior=True)  # based on the results in the tests for submission 8, manually use these params
nbModel.fit(trainSamples, trainLabels)
predictions = nbModel.predict(testSamples)
g = open("../output/vectorizeModifiedParamsBayes.txt", 'w')
g.write("id,label\n")
